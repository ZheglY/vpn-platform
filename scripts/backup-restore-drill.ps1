...
try {
    ...
    foreach ($database in $databases) {
        ...
        Invoke-Compose @(
            ...
            "--database", $database.name, "--owner", $database.owner,
            "--output", "/backups/$($database.name).source.json"
        )
    }
    ...
    & docker compose -p $project @profiles down -v --remove-orphans 2>$null | Out-Null
    ...
    if ($resolvedArtifacts.StartsWith($resolvedRepoTmp + [System.IO.Path]::DirectorySeparatorChar, [System.StringComparison]::OrdinalIgnoreCase)) {
