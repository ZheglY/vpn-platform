...
- The Debian 12 Ansible baseline enforces WireGuard-only management, nftables default deny, separate non-root node-agent/Xray systemd identities, restricted credentials/sudo, idempotence, and last-known-good rollback in disposable Linux tests.
...
- Bounded load and failure injection cover subscription latency/error budgets, Kafka replay without duplicate entitlement, PostgreSQL liveness/readiness recovery, real failover VPN traffic after primary loss, Xray rollback, and Provisioning reconciliation.
