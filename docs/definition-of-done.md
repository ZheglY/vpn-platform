...
- SLI recording rules define eligible and bad events for control APIs, the subscription endpoint, and initial payment-to-provisioning delay. Multi-window burn and p95 alerts have `promtool` tests; Alertmanager validates with inert credential-free receivers and makes no production delivery claim.
- Retention commands are owner-local, dry-run-first, bounded, legal-hold aware, and tested against PostgreSQL. Payment and correctness records, fresh rows, and unresolved dead letters are protected.
- Every owner database has an age-encrypted streaming backup and clean restore drill with ciphertext integrity, exact public-table row-count, relation-owner, RPO, and RTO evidence. Temporary key material and artifacts are removed.
- Stage 8 slices 1-6 are implemented. Node hardening, secret rotation, load/chaos, SBOM, signing, provenance, and production deployment artifacts still require executable tests or drill evidence before Stage 8 acceptance.
