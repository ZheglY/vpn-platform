# Техническое задание: микросервисная VPN-платформа на Go

## 0. Назначение документа

Этот файл фиксирует требования, архитектурные ограничения и поэтапный план реализации production-платформы. Изменения выполняются проверяемыми инкрементами с обязательной проверкой diff, тестов, контрактов и документации.

Локально компоненты запускаются через Docker Compose, но границы сервисов должны позволять независимый production-деплой.

---

## 1. Основные инженерные требования

Платформа продает и выдает VPN-доступ через Telegram-бота. Клиентское приложение пользователя — Happ. Happ не является VPN-провайдером: платформа выдает секретную subscription URL, совместимую с Happ, а VPN-трафик проходит через управляемые узлы с Xray-core. Базовый протокол первой версии — VLESS + REALITY.
...
1. Прочитать `docs/VPN_PLATFORM_SPEC.md`, `CONTRIBUTING.md` и применимые ADR.
2. Исследовать репозиторий и актуальные официальные спецификации интеграций.
3. Зафиксировать неизвестные продуктовые решения в `docs/adr/` как вопросы или ADR. Не выдумывать юридические, налоговые и provider-specific значения.
4. Составить план текущего этапа: файлы, миграции, API/events, тесты, риски и rollback.
5. Изменения публичного API, схем событий, модели безопасности, платежной семантики или границ сервисов требуют отдельного подтверждения.
...
Работы выполняются по одному утвержденному этапу без автоматического перехода к следующему.
...
Использовать микросервисы в монорепозитории. Это не монолит: каждый сервис имеет собственный binary, bounded context, конфигурацию, миграции, Docker image, API и владельца данных. Монорепозиторий упрощает CI, локальный запуск и согласованное изменение контрактов.
...
Версии библиотек из этого документа не являются источником истины: перед изменением необходимо проверить актуальные стабильные версии и changelog, затем закрепить конкретные версии в репозитории.
...
- `CONTRIBUTING.md`, `PLANS.md`, Definition of Done;
...
## 17. Правила внесения изменений

`CONTRIBUTING.md` должен оставаться кратким и ссылаться на это техническое задание. Минимальное содержание:

```markdown
# Repository rules

- Read `docs/VPN_PLATFORM_SPEC.md` before architectural or security-sensitive work.
...
До реализации должны быть заданы и зафиксированы ответы минимум на эти вопросы:
...
- Architecture, threat model, API/events, operations и production README соответствуют коду.

---

## 21. Официальные источники

Документация меняется; перед реализацией интеграции необходимо проверить актуальную официальную страницу, а не полагаться только на этот файл.
...
## 22. Итоговые требования к качеству

Качество продукта определяется не количеством сервисов и технологий, а корректностью границ, платежной идемпотентностью, безопасным lifecycle credential, воспроизводимостью инфраструктуры и проверяемостью отказов. Production-quality нельзя подменять пустыми интерфейсами, boilerplate или диаграммами. Каждый добавленный механизм должен иметь реальный use case, тест, метрику и понятную эксплуатационную модель.

Этапы выполняются последовательно. До получения ответов на вопросы, влияющие на деньги, право, privacy и способ отзыва доступа, необратимые контракты не реализуются.
