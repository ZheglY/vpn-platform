...
Generated files are local secrets and must not be committed. Default Stage 2 Compose mounts them read-only and configures identity-service server TLS through `httpserver.NewMutualTLSConfig`; telegram-bot uses the generated client certificate for identity-service calls.
