...
8. Stage 6 pins official Xray-core `26.3.27` source at commit `d2758a023cd7f4174a5a5fa4ff66e487d4342ba0`. The build downloads that commit archive, verifies SHA-256 `14fa566ee0a801d3d51144c67018b449f5dcf462ddfacadd032da069787e61f9`, and compiles it with the pinned Go `1.26.5` builder. The official release binary is not shipped because its Go `1.26.1`, `golang.org/x/crypto v0.49.0`, and `golang.org/x/net v0.52.0` components contain HIGH vulnerabilities with vendor fixes. The rebuild raises only reviewed security dependencies; `go mod tidy`, `go mod verify`, the image vulnerability scan, and the real data-plane smoke are required gates. It pins `x/crypto v0.53.0`, `x/net v0.56.0`, and, after the 2026-08-02 Trivy gate detected `CVE-2026-56852` in `x/text v0.37.0`, `x/text v0.40.0`. On 2026-07-23, GHSA-hrxh-6v49-42gf made every gRPC-Go version below `1.82.1` fail the HIGH image gate, so the rebuild also pins `google.golang.org/grpc v1.82.1`. The latest stable official Xray release remained `26.3.27` during both reviews; retaining its integrity-pinned source and applying narrow dependency updates avoids an unrelated data-plane upgrade. The decision was checked against the [official release list](https://github.com/XTLS/Xray-core/releases), [official build instructions](https://github.com/XTLS/Xray-core), [gRPC-Go advisory](https://github.com/advisories/GHSA-hrxh-6v49-42gf), [official `x/text` module](https://pkg.go.dev/golang.org/x/text@v0.40.0), [official command documentation](https://xtls.github.io/en/document/command.html), [official REALITY transport documentation](https://xtls.github.io/en/config/transports/reality.html), and the [pinned VLESS config source](https://github.com/XTLS/Xray-core/blob/v26.3.27/infra/conf/vless.go). The renderer deliberately emits inbound `clients`, which is the field accepted by this pin; a future Xray upgrade must re-check schema compatibility instead of following rolling website examples blindly.
