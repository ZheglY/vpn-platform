...
| node-agent compromise becomes root or Xray identity compromise | Debian deployment separates non-root users, restricts shared files to one group, admits management only through WireGuard, and grants one fixed reload/status helper rather than arbitrary sudo. Real host/provider firewall validation remains a production gate. |
