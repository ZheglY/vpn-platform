...
| Monitoring certificate becomes a general service credential | The generated certificate is client-only and application allowlists grant it only `/metrics`; it is not accepted by business endpoints. Rotation order is documented and drilled; production issuance, expiry alerting, and revocation authority remain pending. |
| Host key ownership makes non-root Linux containers fail or encourages world-readable permissions | Runtime services never bind-mount host keys. A network-isolated allowlist init writes per-owner named volumes with `0400`/`0440`; capability-free UID 65532 verification gates startup on native Linux. The nginx camouflage key remains isolated and root-owned for its root master. Production secret injection remains a Stage 9 gate. |
...
| Observability image ships vulnerable or unnecessary code | Prometheus, Grafana, Tempo, and Loki builds pin source/runtime integrity, verify modules/licenses, update reviewed vulnerable dependencies, reduce unused code where practical, and require zero unsuppressed HIGH/CRITICAL image findings. Grafana omits unused bundled Zipkin/Elasticsearch executables and includes only the Tempo protobuf DTO tree needed by its server graph. |
| Effective observability configuration discloses an authentication secret | Tempo accepts Prometheus remote-write configuration and exposes `/status/config`; its compatible Prometheus LTS dependency must use the patched redacting `prometheus/common/config.Secret` type for Azure AD client credentials. The Docker build asserts the exact type. One exact-PURL VEX corrects Trivy's omission of the patched LTS branch and remains visible; all other findings fail. |
...
| Caller-controlled propagation leaks data through baggage | The global propagator is W3C TraceContext only. Baggage and vendor-specific propagators are disabled; HTTP and Kafka instrumentation inject only `traceparent` and optional `tracestate`. |
| Raw URL, Kafka payload, or error text enters a span | HTTP spans use bounded method, registered route, and status only. Kafka spans use only system and fixed operation. Tests inject bearer-like paths, query data, keys, and payloads and assert they are absent. |
...
| Retention reports become an identifier side channel or hide partial deletion | Reports contain only owner, fixed dataset, PostgreSQL cutoff, aggregate eligible/protected/deleted counts, and bounds. A later failure preserves earlier totals and adds only bounded dataset/stage. They contain no row ID, payload, hold reason, user, payment, credential, event, SQL, or database error. |
...
| Corrupt, inconsistent, partial, or wrong-owner backup is accepted as recoverable | Source inspection and `pg_dump` share one exported repeatable-read snapshot. Restore verifies ciphertext SHA-256, refuses every non-empty target before `pg_restore`, uses one transaction, and compares exact public-table row counts and relation owners. Integrity or ownership mismatch fails the drill. |
...
| node-agent compromise becomes root or Xray identity compromise | Debian deployment separates non-root users and primary groups, grants only a narrow supplementary config-sharing group, verifies key access under actual service identities, admits management only through WireGuard, and grants one fixed reload/status helper rather than arbitrary sudo. The role owns only `inet vpn_node` and preserves other firewall tables. Real host/provider firewall validation remains a production gate. |
| A primary-node outage is hidden by healthy control-plane probes | The resilience drill stops the active primary and requires real VLESS + REALITY traffic through the separately provisioned failover before recovery. This is local evidence, not a regional availability claim. |
| Old key generations remain valid indefinitely | Versioned Access keyrings are bounded to four, mTLS/provider/REALITY procedures have explicit overlap and retirement, and the drill covers rollback. Production custody and compromise-specific revocation remain open. |
| Release tooling, omitted Dockerfiles, substituted reports, extra files, or mutable tags replace reviewed image bytes | Release tool images and Actions are digest/commit pinned. Inventory classifies every custom production Dockerfile; Syft and Trivy resolve immutable image IDs; the verifier checks the SPDX container subject and Trivy artifact against that ID, binds report checksums, and rejects extra artifacts; repository-bound OIDC provenance fails closed. Registry publication by immutable digest remains Stage 9 work. |
