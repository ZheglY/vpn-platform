...
Status: implementation and local evidence complete; ready for review on `codex/stage9-production-readiness`. Heavy drills and the release bundle are bound to `f4c7d1eb500944b80d6b8749e69a8427468dbd2d`; final full verification and review controls are bound to `12fef12063b657639fd4d4ef377bba29c4b1633e`. The branch is based on the accepted Stage 8 commit `76f5624e664c556c3ec598055524319eab0af1f3`. The production decision is `NO-GO` because the explicitly external legal, provider, license, PKI, HA/PITR, custody, alerting, registry, capacity, and production-like game-day gates remain blocked.
...
1. **Inventory and admission.** Verify the accepted Stage 8 baseline and remediation controls, then inventory service/data ownership, HTTP and Kafka contracts, public ingress, mTLS and management boundaries, observability, release images, workflows, and open risks in `docs/reviews/stage9/system-inventory.md`. Checks: clean baseline, upstream equality, focused keyring/loadprobe tests, and disposable node hardening. Risk: accidentally treating local Compose as production topology. Rollback: documentation-only revert.
2. **Architecture and code review.** Review bounded contexts, imports, databases/grants, HTTP/Kafka decisions, outbox/inbox, idempotency, ordering, leases/fencing, payment/subscription/access/provisioning/notification/admin state, contracts, HTTP limits, shutdown, health, money types, and placeholders. Record evidence and findings in `docs/reviews/stage9/architecture-code-review.md`; no P0/P1 may remain open. Checks: architecture scans plus existing unit, PostgreSQL, race, contract, migration, Compose, VPN, and Stage 7 suites. Risk: false confidence from static scans. Rollback: remove review evidence; no runtime change is planned.
3. **Security and privacy review.** Re-evaluate every trust boundary, authentication/authorization matrix, PKI, secret custody/injection, edge redaction, SSRF, request bounds, TLS, networks, Kafka ACL, DB roles, container hardening, telemetry, backups, data-subject workflows, abuse, and lawful-request handling. Update `docs/threat-model.md`, `docs/risk-register.md`, and create `docs/reviews/stage9/security-privacy-review.md`. Risk: inventing an unapproved security boundary or retention policy. Rollback: revert review-only additions; any behavioral correction requires separate approval.
4. **Dependency and license review.** Reconcile Go/npm/container/tool/workflow dependencies against the 19-image release inventory and generated SPDX artifacts. Create `docs/reviews/stage9/dependency-license-review.md` and a minimal machine-readable review policy only if it can fail closed without claiming legal advice. Checks: module verification, npm audit, image build/scan, SBOM subject/checksum validation, license evidence, and unknown-license gate. Risk: scanner metadata is incomplete or legally ambiguous. Rollback: retain `NO-GO`; do not weaken the policy.
5. **Legal, payment, privacy, and provider checklist.** Create `docs/reviews/stage9/legal-payment-provider-checklist.md` with only `resolved`, `blocked`, `not_applicable`, or `requires_counsel`, named owners, evidence, review dates, and launch-blocking flags. Unknown jurisdiction, tax, receipt, privacy, YooKassa, Telegram, VPS AUP, sanctions, support, abuse, and lawful-request decisions remain blockers rather than assumptions. Rollback: documentation-only; sandbox restrictions remain mandatory.
6. **Production infrastructure readiness.** Create `docs/reviews/stage9/production-topology-readiness.md` covering domains/edge, DDoS, HA/PITR, Kafka/Redis, secret manager, PKI/revocation, admin custody, immutable off-host backup, observability, alert delivery, on-call, capacity, VPS/WireGuard, registry, signatures/provenance, digest deployment, canary, rollback, DR, and configuration ownership. Present options without selecting providers. Risk: local evidence being mistaken for a production design. Rollback: keep every unselected dependency as `NO-GO`; do not deploy.
7. **Incident game day.** Add `docs/runbooks/incident-response.md`, `docs/runbooks/game-day.md`, and `docs/reviews/stage9/game-day-report.md`. Execute all locally safe scenarios and explicitly block PostgreSQL HA failover, production-like PKI revocation, real alert delivery, and other unavailable staging controls. Checks include ambiguous payment, Kafka replay, real VLESS + REALITY failover, token rotation/redaction, certificate rejection, admin compromise containment, Xray last-known-good, eight-database restore, release rollback, and observability-backend isolation. Risk: destructive fault injection. Rollback: unique Compose namespaces, exact cleanup ownership, abort conditions, and no production targets.
8. **Rollback and recovery.** Create `docs/runbooks/production-rollback.md` for immutable digests, canary abort, forward-only expand/migrate/contract, Kafka compatibility, payment/entitlement/access reconciliation, node/Xray last-known-good, REALITY and mTLS overlap, token compromise, DNS/edge, backup restore, and full DR. Exercise only safe local rollback paths. Risk: implying automatic database downgrade. Rollback: forward fix or isolated restore only.
9. **Go/no-go and evidence.** Create `docs/reviews/stage9/go-no-go-checklist.md` and `docs/reviews/stage9/evidence-index.md` with commit-bound commands, aggregate secret-free results, owners, freshness, and explicit blockers. Add a small `make production-readiness` document/control validator if it materially reduces evidence drift. The decision must be `NO-GO` whenever a hard gate is open.
10. **Final verification and publication.** Run `make production-readiness`, `make verify`, all required Compose/VPN/Stage 7/observability/backup/node/rotation/resilience drills, and `make release-bundle`; review the entire diff for correctness, security, privacy, compatibility, destructive behavior, and Windows/Linux portability. Keep heavy reports ignored and checksum-bound. Commit in small logical units and push only after a clean-tree final gate. Do not merge, publish images, attest a production release, or deploy.

Stage 9 acceptance:

- Every requested review and runbook exists, is tied to concrete evidence, and distinguishes local proof from unavailable production/staging proof.
- Architecture/security review has no open P0/P1. Any deferred P2 has an owner, due date, rationale, and explicit product-owner acceptance; otherwise it remains blocking.
- Unknown/prohibited license or unmet distribution obligation is blocking.
- Legal, payment, privacy, VPS/provider, production PKI/revocation, secret custody, HA/PITR, off-host backup, real alert delivery, registry publication, capacity, abuse/support, and staging game-day gaps are represented as hard gates.
- All mandatory local checks pass. A failed or unavailable production/staging check is never rewritten as success.
- Final Stage 9 completion may report `NO-GO`; it does not grant launch or deployment authority.
